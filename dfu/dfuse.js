...
(function() {
    ...
    dfuse.Device.prototype.do_download = async function(xfer_size, data, manifestationTolerant) {
        ...
        // Inform any consumer that we're starting the erase phase
        try { if (typeof this.onPhaseChange === 'function') this.onPhaseChange('Erasing'); } catch (e) {}
        ...
        // Switch to flashing/write phase
        try {
            // Slightly delay transitioning to 'Flashing' so erase completion can settle and display correctly
            if (typeof this.onPhaseChange === 'function') {
                setTimeout(() => { try { this.onPhaseChange('Flashing'); } catch (e) {} }, 40);
            }
        } catch (e) {}
