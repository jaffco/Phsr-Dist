...
This repository includes a small walkthrough landing UI that prompts the user to link a JuTron device and automatically flashes `firmware/JuTron.bin` when the user clicks the **Link** button. The UI styling borrows design tokens (colors, font) from `jaffx.audio` which is included as a subdirectory in the workspace.
